	...
	Brahma.component('wheel', {
		...
		handle: function(element, delta) {
			if (delta>0) {
				this.trigger('up',[element]);
				if ("function"==typeof this.config.wheelUp) this.config.wheelUp(element);
			}
			if (delta<0) {
				this.trigger('down',[element]);
				if ("function"==typeof this.config.wheelDown) this.config.wheelDown(element);
			}
		},
		...
		execute: function() {
			...
			(function(el) {
				...
				Brahma(el)[0].onmouseenter = function() {
					that.current.overed = true;
				};
				
				Brahma(el)[0].mouseleave = function() {
